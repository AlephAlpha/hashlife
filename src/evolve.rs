...
impl World {
    ...
    fn step_id(&mut self, id: NodeId) -> Node {
        ...
        let node = if self.step + 2 == data.level {
            ...
            match data.children {
                QuadChildren::Leaf { nw, ne, sw, se } => self.step_quad_leaf(nw, ne, sw, se),
                QuadChildren::NodeId { nw, ne, sw, se } => self.step_quad(nw, ne, sw, se),
            }
        };
            ...
            None => {
                let node = match data.children {
                    QuadChildren::Leaf { nw, ne, sw, se } => {
                        self.step_quad_leaf_max(nw, ne, sw, se)
                    }
                    QuadChildren::NodeId { nw, ne, sw, se } => self.step_quad_max(nw, ne, sw, se),
                };
    ...
    fn step_quad_leaf(&self, nw: Leaf, ne: Leaf, sw: Leaf, se: Leaf) -> Node {
    ...
    fn step_quad_leaf_max(&self, nw: Leaf, ne: Leaf, sw: Leaf, se: Leaf) -> Node {
        let t00 = self.step_leaf(nw);
        let t01 = self.step_leaf((nw & 0x3333) << 2 | (ne & 0xcccc) >> 2);
        let t02 = self.step_leaf(ne);
        let t10 = self.step_leaf((nw & 0x00ff) << 8 | (sw & 0xff00) >> 8);
    ...
    fn step_quad(&mut self, nw: NodeId, ne: NodeId, sw: NodeId, se: NodeId) -> Node {
    ...
    fn step_quad_max(&mut self, nw: NodeId, ne: NodeId, sw: NodeId, se: NodeId) -> Node {
        let n01 = self.find_node(self[nw].ne(), self[ne].nw(), self[nw].se(), self[ne].sw());
        let n10 = self.find_node(self[nw].sw(), self[nw].se(), self[sw].nw(), self[sw].ne());
        let n11 = self.find_node(self[nw].se(), self[ne].sw(), self[sw].ne(), self[se].nw());
        let n12 = self.find_node(self[ne].sw(), self[ne].se(), self[se].nw(), self[se].ne());
    ...
    fn test_level_3() {
        ...
        macro_rules! test_level_3 {
            ($nw:expr, $ne:expr, $sw:expr, $se:expr, $result1:expr, $result2:expr $(,)?) => {{
                let result1 = world.step_quad_leaf($nw, $ne, $sw, $se);
                assert_eq!(result1, Node::Leaf($result1));
                let result2 = world.step_quad_leaf_max($nw, $ne, $sw, $se);
